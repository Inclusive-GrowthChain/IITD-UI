          ...
          <div className="row">
            <div className="col-lg-6">
              <RepaymentContentTitle
                title="Loan ID"
                val={data?.value?.loanId || data?.loanId}
              />
              <RepaymentContentTitle
                title="Farmer Name"
                val={data?.value?.name || data?.userName || data?.name || data?.key}
              />
              <RepaymentContentTitle
                title="FPO Name"
                val={data?.value?.fpoName || data?.fpoName}
              />
              <RepaymentContentTitle
                title="Loan Amount"
                val={data?.value?.grantedAmount || data?.grantedAmount}
              />
            </div>
            ...
            <div className="col-lg-6">
              <RepaymentContentTitle
                ...
                val={data?.value?.loanTenure || data?.loanTenure}
              />
              <RepaymentContentTitle
                title="No of Repayment"
                val={
                  data?.value?.farmerWindowRepaymentStructure?.length ||
                  data?.farmerWindowRepaymentStructure.length
                }
              />
              <RepaymentContentTitle
                title="Annual Interest Rate"
                val={`${data?.value?.intrest || data?.intrest}%`}
              />
            </div>
          </div>
        </div>
                ...
                return (
                  <tr key={key}>
                    ...
                    <td>
                      <button
                        disabled={
                          r.completed || repaymentDateTime >= currentDate
                        }
                        className="py-0.5"
                        style={btnStyle}
